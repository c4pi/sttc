...
def test_get_executable_path_dev_headless(monkeypatch) -> None:  # type: ignore[no-untyped-def]
    ...
    monkeypatch.setattr(autostart.sys, "executable", r"C:/Python313/python.exe")

    assert autostart.get_executable_path() == "C:/Python313/python.exe -m sttc run"
...
def test_get_executable_path_dev_gui_minimized(monkeypatch) -> None:  # type: ignore[no-untyped-def]
    ...
    assert autostart.get_executable_path(gui=True, minimized=True) == "C:/Python313/python.exe -m sttc run --gui --minimized"
...
def test_get_executable_path_quotes_source_python_when_needed(monkeypatch) -> None:  # type: ignore[no-untyped-def]
    monkeypatch.setattr(autostart, "is_bundled_executable", lambda: False)
    monkeypatch.setattr(autostart.sys, "executable", r"C:/Program Files/Python313/python.exe")

    assert autostart.get_executable_path() == '"C:/Program Files/Python313/python.exe" -m sttc run'
